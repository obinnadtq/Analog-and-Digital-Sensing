...
class SequentialDistributedIB(GenericAIB):
    ...
    def __init__(self, Nsensors_, p_xyi_, p_init, Nx_, Ny_, Nz_, target_rates_, beta_range, params):
        ...
        if self.params.get('ib_solver') == 'iterative':
            for idx in range(0, self.Nsensors):
                solver_list_no_sideinfo_[idx] = IterativeIBScalar(p_xyi_[idx], self.p_init[idx], self.Nx, self.Ny[idx],
                                                                  self.Nz[idx], None, self.params.get('accuracy_ib'),
                                                                  self.params.get('max_iter_ib'))
            self.solver_list_no_sideinfo = solver_list_no_sideinfo_
            self.solver_sideinfo = SequentialDistributedIterativeIBSideinformation(self.Nsensors,
                                                                                   self.params.get('sensor_opt_order'),
                                                                                   p_xyi_, self.p_init, self.Nx,
                                                                                   self.Ny, self.Nz, None,
                                                                                   self.params.get('accuracy_ib'),
                                                                                   self.params.get('max_iter_ib'))
        ...
        for aib_idx in range(0, self.params.get('max_iter_aib')):
            ...
            for node_idx in range(0, self.Nsensors):
                ...
                node_results[node_idx]['I_xyi'] = np.sum(self.p_yix[node_idx] * np.log2(self.p_yix[node_idx] \
                                                                                        / (np.tile(
                    np.expand_dims(self.p_x, 0), np.append(self.Ny[node_idx], 1)) * np.tile(
                    np.expand_dims(self.p_yi[node_idx], -1), np.append(1, self.Nx)))))
                ...
                for bs_idx in range(0, self.params.get('max_iter_bs')):
                    ...
                    node_bs_iteration_results[bs_idx]['residual_ib'] = [None for _ in
                                                                        range(0, self.params.get('max_iter_ib'))]
                    ...
                    if self.params.get('store_inner_ib_results'):
                        node_bs_iteration_results[bs_idx]['I_yizi_zj_evolution'] = [None for _ in range(0,
                                                                                                        self.params.get(
                                                                                                            'max_iter_ib'))]
                        node_bs_iteration_results[bs_idx]['I_xzi_zj_evolution'] = [None for _ in range(0,
                                                                                                       self.params.get(
                                                                                                           'max_iter_ib'))]
                        node_bs_iteration_results[bs_idx]['I_yizi_evolution'] = [None for _ in range(0, self.params.get(
                            'max_iter_ib'))]
                        node_bs_iteration_results[bs_idx]['I_xzi_evolution'] = [None for _ in range(0, self.params.get(
                            'max_iter_ib'))]
                        node_bs_iteration_results[bs_idx]['H_zi_evolution'] = [None for _ in
                                                                               range(0, self.params.get('max_iter_ib'))]
                        node_bs_iteration_results[bs_idx]['I_xz_evolution'] = [None for _ in
                                                                               range(0, self.params.get('max_iter_ib'))]
                        node_bs_iteration_results[bs_idx]['H_z_evolution'] = [None for _ in
                                                                              range(0, self.params.get('max_iter_ib'))]
                        node_bs_iteration_results[bs_idx]['H_zi_zj_evolution'] = [None for _ in range(0,
                                                                                                      self.params.get(
                                                                                                          'max_iter_ib'))]
        ...
        while ((residual_aib > self.params.get('accuracy_aib')) and (iter_aib < self.params.get('max_iter_aib'))):
            ...
            for sensor_idx in opt_order:
                ...
                while ((residual_bs > self.params.get('accuracy_bs')) and (iter_bs < self.params.get('max_iter_bs'))):
                    ...
                    beta = np.mean(lambda_bounds)
                    self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                        'beta'] = beta
                    ...
                    while ((residual_ib > self.params.get('accuracy_ib')) and (
                            iter_ib < self.params.get('max_iter_ib'))):
                        ...
                        if iter_ib % 10 == 0:
                            logging.info('SIB::' + '\t'
                                         + 'Permutation: ' + str(
                                self.params.get('sensor_opt_order')) + ' iterIB: ' + str(iter_ib) + ' iterBS: ' + str(
                                iter_bs) + ' node: ' + str(sensor_idx) + ' iterSIB: ' + str(
                                iter_aib)
                                         + '\t' + 'optimized with side-information: ' + str(
                                bool(self.params.get('sensor_opt_with_side')[sensor_idx])))

                        if self.params.get('sensor_opt_with_side')[sensor_idx]:
                            p_zi_yi[sensor_idx], p_zi_x[
                                sensor_idx], p_zx, p_x_z, p_z = self.solver_sideinfo.run_one_iteration(sensor_idx,
                                                                                                       p_zi_yi, p_zi_x,
                                                                                                       beta, iter_ib)

                        else:
                            p_zi_yi[sensor_idx], p_zi_x[sensor_idx] = self.solver_list_no_sideinfo[
                                sensor_idx].run_one_iteration(p_zi_yi[sensor_idx], beta)

                        if self.params.get('store_inner_ib_results'):
                            if self.params.get('sensor_opt_with_side')[sensor_idx]:
                                self.determine_mutual_information(p_zi_yi, p_zi_x, p_zx, p_x_z, p_z, sensor_idx,
                                                                  iter_aib, iter_bs, iter_ib=iter_ib)
                            else:
                                self.determine_mutual_information(p_zi_yi, p_zi_x, p_zx=None, p_x_z=None, p_z=None,
                                                                  sensor_idx=sensor_idx, iter_aib=iter_aib,
                                                                  iter_bs=iter_bs, scalar=True, iter_ib=iter_ib)

                        # calculate residual
                        residual_ib = self.calculate_residual_IB(p_zi_yi[sensor_idx], p_zi_yi_prev_ib,
                                                                 self.params.get('ib_conv_crit'), sensor_idx)

                        self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][
                            iter_bs]['residual_ib'][iter_ib] = residual_ib
                        iter_ib += 1
                    # ------------------------------------------------------------------------------------------------

                    self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                        'used_iter_ib'] = iter_ib

                    # determine mutual information and calculate accuracy of bisection search and adjusting beta
                    if self.params.get('sensor_opt_with_side')[sensor_idx]:
                        self.determine_mutual_information(p_zi_yi, p_zi_x, p_zx, p_x_z, p_z, sensor_idx, iter_aib,
                                                          iter_bs)

                        if sensor_idx != self.params.get('sensor_opt_order')[0]:
                            residual_bs, lambda_bounds = self.calculate_residual_BS(self.target_rates[sensor_idx],
                                                                                    self.aib_result['Iterations_aib'][
                                                                                        iter_aib]['Node_results'][
                                                                                        sensor_idx]['Iterations_bs'][
                                                                                        iter_bs]['I_yizi_zj'], False,
                                                                                    lambda_bounds)
                        else:
                            residual_bs, lambda_bounds = self.calculate_residual_BS(self.target_rates[sensor_idx],
                                                                                    self.aib_result['Iterations_aib'][
                                                                                        iter_aib]['Node_results'][
                                                                                        sensor_idx]['Iterations_bs'][
                                                                                        iter_bs]['I_yizi'], False,
                                                                                    lambda_bounds)
                    else:
                        self.determine_mutual_information(p_zi_yi, p_zi_x, p_zx=None, p_x_z=None, p_z=None,
                                                          sensor_idx=sensor_idx, iter_aib=iter_aib, iter_bs=iter_bs,
                                                          scalar=True)
                        residual_bs, lambda_bounds = self.calculate_residual_BS(self.target_rates[sensor_idx],
                                                                                self.aib_result['Iterations_aib'][
                                                                                    iter_aib]['Node_results'][
    ...
    def determine_p_zi_x(self, p_zi_yi, sensor_idx=None):
        ...
        p_zi_yi_final = []
        for idx in range(0, self.Nsensors):
            pzbari_zi = np.ones(np.append(self.Nz[idx], self.Nz[idx]))
            pe = 0
            for idx1 in range(len(pzbari_zi)):
                for idx2 in range(len(pzbari_zi)):
                    if idx1 == idx2:
                        pzbari_zi[idx1, idx2] = 1 - pe
                    else:
                        pzbari_zi[idx1, idx2] = pe / (self.Nz[idx] - 1)
            obi.append(pzbari_zi)
        for idx1 in range(0, self.Nsensors):
            temp = np.sum(
                np.tile(np.expand_dims(obi[idx1], axis=2), (1, 1, self.Ny[idx1])) * np.tile(
                    np.expand_dims(p_zi_yi[idx1], axis=0),
                    (self.Nz[idx1], 1, 1)), axis=1)
            p_zi_yi_final.append(temp)
        p_zi_x = [[] for _ in range(0, self.Nsensors)]
        for idx in range(0, self.Nsensors):
            p_ziyixi = np.tile(np.expand_dims(self.p_yix[idx], 0),
                               np.append(self.Nz[idx], np.ones(2, dtype=int))) * np.tile(
                np.expand_dims(p_zi_yi_final[idx], -1), self.Nx)
    ...
    def determine_sum_rate(self, p_zi_yi, init=False, p_zi_x=None, sensor_idx=None, iter_aib=None, iter_bs=None,
                           p_zx=None, p_x_z=None, iter_ib=None, scalar=False):
        ...
        if init:
            ...
            for idx in range(0, self.Nsensors):
                p_ziyixi = np.tile(np.expand_dims(self.p_yix[idx], 0),
                                   np.append(self.Nz[idx], np.ones(2, dtype=int))) * np.tile(
                    np.expand_dims(p_zi_yi_final[idx], -1), self.Nx)
                p_zi_x[idx] = (1 / np.tile(np.expand_dims(self.p_x, 0), np.append(self.Nz[idx], 1))) * np.sum(p_ziyixi,
                                                                                                              1)

            p_zx = np.ones(np.append(self.Nz, self.Nx))
            p_x_expand = np.tile(self.increase_dims(self.p_x, np.arange(0, self.Nsensors, dtype=int)),
                                 np.append(self.Nz, 1))
            for s_idx in range(0, self.Nsensors):
                ...
                p_zi_x_expand = np.tile(self.increase_dims(p_zi_x[s_idx], j_elements),
                                        self.put_in_position(np.ones(self.Nsensors + 1, dtype=int), j_elements,
                                                             np.array(self.Nz)[j_elements]))
        ...
        if init:
            ...
            if scalar:
                if iter_ib == None:
                    ...
                    self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                        'I_xz_evolution'][iter_ib] = np.sum(tmp)
        else:
            if iter_ib == None:
                self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                    'I_xz'] = np.sum(tmp)
    ...
    def determine_mutual_information(self, p_zi_yi, p_zi_x, p_zx, p_x_z, p_z, sensor_idx, iter_aib, iter_bs,
                                     scalar=False, iter_ib=None):
        ...
        for idx1 in range(0, self.Nsensors):
            ...
            p_zi_yi_final.append(temp)
        # determine I(Yi;Zi)
        p_ziyi = p_zi_yi_final[sensor_idx] * np.tile(np.expand_dims(self.p_yi[sensor_idx], 0),
                                                     np.append(self.Nz[sensor_idx], 1))
        ...
        if iter_ib == None:
            ...
            self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                'I_yizi_evolution'][iter_ib] = np.sum(tmp)
        tmp = None
        tmp1 = None
        tmp2 = None
        ...
        if iter_ib == None:
            ...
            self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                'I_xzi_evolution'][iter_ib] = np.sum(tmp)
        tmp = None
        tmp1 = None
        tmp2 = None
        ...
        if iter_ib == None:
            ...
            self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                'H_zi_evolution'][iter_ib] = - np.sum(tmp)
        tmp = None

        if scalar or (self.params.get('chain_optimization') and self.params.get('sensor_opt_order')[0] == sensor_idx):
            # determine I(X;Z)
            if iter_ib == None:
                self.determine_sum_rate(p_zi_yi, init=True, sensor_idx=sensor_idx, iter_aib=iter_aib, iter_bs=iter_bs,
                                        scalar=True)
            else:
                self.determine_sum_rate(p_zi_yi, init=True, sensor_idx=sensor_idx, iter_aib=iter_aib, iter_bs=iter_bs,
                                        iter_ib=iter_ib, scalar=True)
            ...
            if iter_ib == None:
                ...
                self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                    'I_yizi_zj_evolution'][iter_ib] \
                    = self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                    'I_yizi_evolution'][iter_ib]
            ...
            if iter_ib == None:
                ...
                self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                    'H_z_evolution'][iter_ib] = - np.sum(tmp)
            tmp = None

            # determine I(X;Z)
            if iter_ib == None:
                self.determine_sum_rate(p_zi_yi, init=False, p_zi_x=p_zi_x, sensor_idx=sensor_idx, iter_aib=iter_aib,
                                        iter_bs=iter_bs, p_zx=p_zx, p_x_z=p_x_z)
            else:
                self.determine_sum_rate(p_zi_yi, init=False, p_zi_x=p_zi_x, sensor_idx=sensor_idx, iter_aib=iter_aib,
                                        iter_bs=iter_bs, p_zx=p_zx, p_x_z=p_x_z, iter_ib=iter_ib)
            ...
            if sensor_idx == self.params.get('sensor_opt_order')[0]:
                ...
                p_zi_zj_partial = p_z_partial / np.tile(self.increase_dims(p_zj_partial, [diff_pos]),
                                                        self.put_in_position(np.ones(len(partial_elements), dtype=int),
                                                                             diff_pos, self.Nz[diff_node]))
                ...
                if iter_ib == None:
                    ...
                    self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                        'H_zi_zj_evolution'][iter_ib] = - np.sum(tmp)
                tmp = None
                ...
                if iter_ib == None:
                    ...
                    self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                        'I_yizi_evolution'][iter_ib] \
                    - self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                        'H_zi_evolution'][iter_ib] \
                    + self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                        'H_zi_zj_evolution'][iter_ib]
                    self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                        'I_yizi_zj_evolution'][iter_ib] = I_yizi_zj

            # determine I(X;Zi|Zj~=i) -> mutual information between X and Zi given Zj~=i)
            p_zjx = np.sum(p_zx, sensor_idx)
            p_zj = np.sum(p_zjx, -1)
            with np.errstate(all='ignore'):
                p_x_zj = p_zjx / np.tile(np.expand_dims(p_zj, -1),
                                         np.append(np.ones(self.Nsensors - 1, dtype=int), self.Nx))
                p_x_zj[np.isnan(p_x_zj)] = 0  # remove NaN
                tmp1 = np.log2(p_x_zj)
                tmp1[np.isnan(tmp1)] = 0  # remove NaN
                tmp2 = np.log2(p_x_z)
                tmp2[np.isnan(tmp2)] = 0  # remove NaN
                tmp = p_zx * (tmp2 - np.tile(np.expand_dims(tmp1, sensor_idx),
                                             self.put_in_position(np.ones(self.Nsensors + 1, dtype=int), sensor_idx,
                                                                  self.Nz[sensor_idx])))
                tmp[np.isnan(tmp)] = 0  # remove NaN
            if iter_ib == None:
                self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                ...
                self.aib_result['Iterations_aib'][iter_aib]['Node_results'][sensor_idx]['Iterations_bs'][iter_bs][
                    'I_xzi_zj_evolution'][iter_ib] = np.sum(tmp)
            tmp = None
            tmp1 = None
            tmp2 = None
    ...
    def determine_vector_compression_rate(self, p_zi_yi, p_zi_x):
        ...
        for sensor_idx in range(0, self.Nsensors):
            p_ziyi_vec[sensor_idx] = p_zi_yi[sensor_idx] * np.tile(np.expand_dims(self.p_yi[sensor_idx], 0),
                                                                   np.append(self.Nz[sensor_idx], 1))
    ...
    def determine_compression_rates(self, p_zi_yi, p_zi_x, iter_aib):
        ...
        for sensor_idx in range(0, self.Nsensors):
            ...
            with np.errstate(all='ignore'):
                ...
                tmp2 = np.log2(p_zi_yi[sensor_idx])
                tmp2[np.isnan(tmp2)] = 0  # remove NaN
                tmp = p_ziyi_vec[sensor_idx] * (
                            tmp2 - np.tile(np.expand_dims(tmp1, -1), np.append(1, self.Ny[sensor_idx])))
                tmp[np.isnan(tmp)] = 0  # remove NaN
            I_yizi = np.sum(tmp)
            tmp = None
            tmp1 = None
        ...
        self.aib_result['Iterations_aib'][iter_aib]['final_compression_rates'][
            'partial_sideinformation'] = comp_rates_partial
        ...
        for condition_idx in range(0, len(powerset)):
            ...
            if len(conditions[condition_idx]['variable']) == 1:
                ...
                p_zi_zj = p_z / np.tile(np.expand_dims(p_zj, sensor),
                                        self.put_in_position(np.ones(self.Nsensors, dtype=int), sensor,
                                                             self.Nz[sensor]))
                with np.errstate(all='ignore'):
                ...
                p_zvar_zcond = p_z / np.tile(self.increase_dims(p_zcond, variable),
                                             self.put_in_position(np.ones(self.Nsensors, dtype=int), variable,
                                                                  np.array(self.Nz)[tuple([variable])]))
                with np.errstate(all='ignore'):
    ...
    def calculate_residual_IB(self, Q, Q_prev, method, sensor_idx):
        ...
        if method == 'MAE':
            ...
            residual_ib = np.sum(np.power(Q.flatten() - Q_prev.flatten(), 2)) / (
                        self.Ny[sensor_idx] * self.Nz[sensor_idx])
        elif method == 'KL':
